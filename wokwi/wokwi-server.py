...
async def handle_client(websocket, path):
    ...
    print("Elf file: {}/{}.elf".format(os.getenv('PROJECT_PATH'), os.getenv('PROJECT_NAME')))
    # Send the simulation payload
    await websocket.send(json.dumps({
        "type": "start",
        "elf": base64_file('{}/build/{}.elf'.format(os.getenv('PROJECT_PATH'), os.getenv('PROJECT_NAME'))),
        "espBin": [
            [0x1000, base64_file('{}/build/bootloader/bootloader.bin'.format(os.getenv('PROJECT_PATH')))],
            [0x8000, base64_file('{}/build/partition_table/partition-table.bin'.format(os.getenv('PROJECT_PATH')))],
            [0x10000, base64_file('{}/build/{}.bin'.format(os.getenv('PROJECT_PATH'), os.getenv('PROJECT_NAME')))],
        ]
    }))
